...
# Mediapipe 관련 모듈 import 및 alias 설정
...
def cleanup_resources():
    ...
    """
    global cap, client, pg, hands_detector
    print("정리 작업을 수행합니다...")
    ...
    # Mediapipe hands 객체 종료 (리소스 해제)
    if 'hands_detector' in globals() and hands_detector is not None:
...
# 메인 설정 및 초기화
# ============================================================
# YOLO 모델 초기화 (사람 detection 및 스켈레톤 추출)
...
# Mediapipe 손 추적 객체 초기화 (최대 2손, 낮은 모델 복잡도)
...
while cap.isOpened():
    ...
    # -------------------------------
    # 2) 사람 객체 중심 좌표 계산 (스켈레톤 또는 BBox 기준)
    # -------------------------------
    try:
    ...
    # -------------------------------
    # 8) 손 검출 및 스켈레톤 표시 (Mediapipe Hand Tracking)
    # -------------------------------
    # BGR 이미지를 RGB로 변환하여 손 검출 수행
    frame_rgb = cv2.cvtColor(reframe, cv2.COLOR_BGR2RGB)
    hands_results = hands_detector.process(frame_rgb)
    if hands_results.multi_hand_landmarks:
        for hand_landmarks in hands_results.multi_hand_landmarks:
            # 손의 스켈레톤 및 연결선 그리기 (annotated_frame 위에 overlay)
